object ObjectOrientation extends App {
  // Usee App to run a simple app / main project
  // java equivalent: public static void main(String[] args)

  // class and instance
  class Animal {
    // define fields
    // similar to java here for a class
    // val is immutable
    val age: Int = 0
  ...
  // "interface" = ultimate abstract type, use to build up classes, use as many as needed
  trait Carnivore {
    def eat(animal: Animal): Unit
  }

  trait Philosopher {
    def ?!(thought: String): Unit // possible method name, don't do this though, unreadable
  }

  // single-class inheritance, multi-trait "mixin"
  ...
  aCroc.eat(aDog) // below line does the same as this
  aCroc eat aDog // infix notation - no dot = object method argument, only available for methods with ONE argument
  ...
  /*
   case classes = lightweight data structures with some boilerplate, use to quickly set up simple classes
   - sensible equals and hash code
   - serialization
   - companion with apply
   - pattern matching
  */
  case class Person(name: String, age: Int) // create your fields here

  // may be constructed without new
  val bob = Person("Bob", 54) // Person.apply("Bob", 54) is the same thing
